    ...
    aspect_ratio(GLfloat(window_width) / (GLfloat)window_height)
{
    ...
    glEnable(GL_DEPTH_TEST);    //  to test fragments through the z-buffer
...
//        waterShaderProgram("waterVertexShader.txt", "waterFragmentShader.txt"),
//        surfaceShaderProgram("surfaceVertexShader.txt", "surfaceFragmentShader.txt"),
...
//        camera(glm::vec3(-0.0f, 1.0f, -15.0f), glm::vec3(0.0f, 0.0f, 1.0f))
{
    // Data follows
    ...
    // Specify vertex z-data
//    glBindBuffer(GL_ARRAY_BUFFER, surfaceVBO[0]);
...
//    glEnableVertexAttribArray(0);
    ...
    glCreateVertexArrays(1, &waterVAO);
...
//    unsigned char *image = SOIL_load_image("water_texture.jpg", &img_width, &img_height, 0, SOIL_LOAD_RGB);
...
namespace ViewUtils {
    ...
    Camera camera(glm::vec3(0.0f, 2.0f, -5.0f), glm::vec3(0.0f, 1.0f, 0.0f), 90.0f);
